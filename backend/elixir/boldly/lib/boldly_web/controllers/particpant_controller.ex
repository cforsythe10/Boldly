defmodule BoldlyWeb.ParticipantController do
  ...
  def create(conn, %{"participant" => participant_params}) do
    with {:ok, %Participant{} = participant} <- CampaignPart.create_participant(participant_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "participant" => participant_params}) do
    ...
    with {:ok, %Participant{} = participant} <- CampaignPart.update_participant(participant, participant_params) do
      render(conn, "show.json", participant: participant)
    end
  end
